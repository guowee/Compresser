...
import com.missile.compress.utils.BitmapUtil;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    int ret = BitmapUtil.compressBitmap(mBuilder.filePath, targetCompressPath, ignoreSize);
                    if (ret == 1) {
                        mHandler.sendMessage(mHandler.obtainMessage(MSG_COMPRESS_SUCCESS, targetCompressPath));
                    } else {
                        mHandler.sendMessage(mHandler.obtainMessage(MSG_COMPRESS_ERROR, new RuntimeException("ERROR")));
                    }
